...
class AppControllerTest {
    ...
    void createPost() throws Exception {
        ...
        // Configurar el mock para que devuelva el post esperado cuando se llame al método con esos parámetros específicos
    ...
    @Test
    void getRootStream() throws Exception {
        // Arrange
        List<Post> rootPosts = Arrays.asList(
                new Post("user1", "Post 1 content"),
                new Post("user2", "Post 2 content")
        );
        rootPosts.get(0).setId("post1");
        rootPosts.get(1).setId("post2");

        when(postService.getRootThreads()).thenReturn(rootPosts);

        // Act & Assert
        mockMvc.perform(get("/posts/stream"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$[0].id").value("post1"))
                .andExpect(jsonPath("$[1].id").value("post2"));

        verify(postService, times(1)).getRootThreads();
    }
